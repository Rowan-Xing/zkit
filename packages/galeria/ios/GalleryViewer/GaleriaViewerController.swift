...
final class GaleriaViewerController: UIViewController {
    private weak var dataSource: GaleriaDataSource?
    private weak var initialSourceView: UIImageView?
    private let imageLoader: GaleriaImageLoading
    private let galleryId: String?
    private let initialIndex: Int
    private let configuration: GaleriaViewerConfiguration
    ...
    private var transitionCoordinatorRef: GaleriaViewerTransitionCoordinator?
    ...
    private weak var dragPageView: GaleriaViewerPageView?
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if hasPerformedOpenTransition == false {
            ...
            scrollToIndex(currentIndex, animated: false)
            transitionCoordinatorRef = GaleriaViewerTransitionCoordinator(containerView: view)
    ...
    private var currentPageView: GaleriaViewerPageView? {
    ...
    private func setupUI() {
        ...
        collectionView.register(GaleriaViewerPageCell.self, forCellWithReuseIdentifier: GaleriaViewerPageCell.reuseIdentifier)
    ...
    private func anchorView(for index: Int) -> UIImageView? {
        if let galleryId = galleryId,
           let sourceView = GaleriaSourceViewRegistry.shared.sourceView(for: galleryId, index: index) {
            return sourceView
        }
    ...
    private func pageView(at index: Int) -> GaleriaViewerPageView? {
        let indexPath = IndexPath(item: index, section: 0)
        return collectionView.cellForItem(at: indexPath).flatMap { $0 as? GaleriaViewerPageCell }?.pageView
    }
    ...
    private func updateVisiblePageStates() {
        for cell in collectionView.visibleCells {
            guard let pageCell = cell as? GaleriaViewerPageCell else { continue }
...
extension GaleriaViewerController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(
            withReuseIdentifier: GaleriaViewerPageCell.reuseIdentifier,
            for: indexPath
        ) as? GaleriaViewerPageCell else {
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        guard let pageCell = cell as? GaleriaViewerPageCell else { return }
...
extension GaleriaViewerController: UIGestureRecognizerDelegate {
...
extension GaleriaViewerController: GaleriaViewerPageViewDelegate {
    func galeriaViewerPageViewDidRequestDismiss(_ pageView: GaleriaViewerPageView) {
        guard pageView.index == currentIndex else { return }
        performDismissTransition()
    }

    func galeriaViewerPageViewDidToggleVideoChrome(_ pageView: GaleriaViewerPageView) {
        guard pageView.index == currentIndex else { return }
        _ = pageView.toggleVideoControls()
        setNavigationBarHidden(shouldHideNavigationBarForCurrentPage, animated: true)
    }

    func galeriaViewerPageView(_ pageView: GaleriaViewerPageView, setHorizontalPagingEnabled enabled: Bool) {
        guard pageView.index == currentIndex else { return }
        recalculateHorizontalPagingEnabled()
    }

    func galeriaViewerPageViewDidBeginMultiTouch(_ pageView: GaleriaViewerPageView) {
        guard pageView.index == currentIndex else { return }
    ...
    func galeriaViewerPageViewDidEndMultiTouch(_ pageView: GaleriaViewerPageView) {
        guard pageView.index == currentIndex else { return }
...
private final class GaleriaViewerPageCell: UICollectionViewCell {
    static let reuseIdentifier = "GaleriaViewerPageCell"

    let pageView = GaleriaViewerPageView()
