...
protocol GaleriaViewerPageViewDelegate: AnyObject {
...
final class GaleriaViewerPageView: UIView {
    weak var delegate: GaleriaViewerPageViewDelegate?

    private(set) var index: Int = 0
    private(set) var item: GaleriaMediaItem?
    ...
    private let videoPlayerView = GaleriaVideoPlayerView(frame: .zero)

    private var imageLoader: GaleriaImageLoading?
    ...
    func sharedElementState() -> GaleriaSharedElementState? {
        if isVideoPage {
            return posterImageView.galeriaSharedElementState()
        }
        return imageView.galeriaSharedElementState(clippingFrameInWindow: imageScrollView.frameInWindow())
    }

    func defaultTransitionGeometry(in windowBounds: CGRect) -> GaleriaSharedElementGeometry {
        ...
        let visibleFrame = GaleriaViewerTransitionCoordinator.aspectFitRect(for: imageSize, in: windowBounds)
    ...
    private func setupUI() {
        ...
        videoPlayerView.onControlsInteractStart = { [weak self] in
            ...
            self.delegate?.galeriaViewerPageView(self, setHorizontalPagingEnabled: false)
        }
        videoPlayerView.onControlsInteractEnd = { [weak self] in
            guard let self = self else { return }
            self.delegate?.galeriaViewerPageView(self, setHorizontalPagingEnabled: self.canPageHorizontally)
        }
    ...
    private func configureImagePage(image: UIImage?) {
        ...
        delegate?.galeriaViewerPageView(self, setHorizontalPagingEnabled: canPageHorizontally)
    }
    ...
    private func configureVideoPage(url: URL, poster: URL?) {
        ...
        delegate?.galeriaViewerPageView(self, setHorizontalPagingEnabled: true)
        updateVideoPlaybackIfNeeded()
    }
    ...
    private func handleImageSingleTap(_ recognizer: UITapGestureRecognizer) {
        delegate?.galeriaViewerPageViewDidRequestDismiss(self)
    }
    ...
    private func handleVideoSingleTap(_ recognizer: UITapGestureRecognizer) {
        delegate?.galeriaViewerPageViewDidToggleVideoChrome(self)
    }
    ...
    private func handlePinchState(_ recognizer: UIPinchGestureRecognizer) {
        ...
        case .began:
            ...
            delegate?.galeriaViewerPageViewDidBeginMultiTouch(self)
        case .cancelled, .ended, .failed:
            if pinchInProgress {
                pinchInProgress = false
                delegate?.galeriaViewerPageViewDidEndMultiTouch(self)
                delegate?.galeriaViewerPageView(self, setHorizontalPagingEnabled: canPageHorizontally)
            }
        default:
            break
...
extension GaleriaViewerPageView: UIScrollViewDelegate {
...
extension GaleriaViewerPageView: UIGestureRecognizerDelegate {
