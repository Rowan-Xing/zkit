...
final class NativeImagePreviewViewerController: UIViewController {
    private weak var dataSource: NativeImagePreviewDataSource?
    private weak var initialSourceView: UIImageView?
    private let imageLoader: NativeImagePreviewImageLoading
    private let previewGroupId: String?
    private let initialIndex: Int
    private let configuration: NativeImagePreviewViewerConfiguration
    ...
    private var transitionCoordinatorRef: NativeImagePreviewViewerTransitionCoordinator?
    ...
    private weak var dragPageView: NativeImagePreviewViewerPageView?
    ...
    init(
        ...
        self.previewGroupId = previewGroupId
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if hasPerformedOpenTransition == false {
            ...
            scrollToIndex(currentIndex, animated: false)
            transitionCoordinatorRef = NativeImagePreviewViewerTransitionCoordinator(containerView: view)
    ...
    private var currentPageView: NativeImagePreviewViewerPageView? {
    ...
    private func setupUI() {
        ...
        collectionView.register(NativeImagePreviewViewerPageCell.self, forCellWithReuseIdentifier: NativeImagePreviewViewerPageCell.reuseIdentifier)
    ...
    private func anchorView(for index: Int) -> UIImageView? {
        if let previewGroupId = previewGroupId,
           let sourceView = NativeImagePreviewSourceViewRegistry.shared.sourceView(for: previewGroupId, index: index) {
            return sourceView
        }
    ...
    private func pageView(at index: Int) -> NativeImagePreviewViewerPageView? {
        let indexPath = IndexPath(item: index, section: 0)
        return collectionView.cellForItem(at: indexPath).flatMap { $0 as? NativeImagePreviewViewerPageCell }?.pageView
    }
    ...
    private func updateVisiblePageStates() {
        for cell in collectionView.visibleCells {
            guard let pageCell = cell as? NativeImagePreviewViewerPageCell else { continue }
...
extension NativeImagePreviewViewerController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(
            withReuseIdentifier: NativeImagePreviewViewerPageCell.reuseIdentifier,
            for: indexPath
        ) as? NativeImagePreviewViewerPageCell else {
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        guard let pageCell = cell as? NativeImagePreviewViewerPageCell else { return }
...
extension NativeImagePreviewViewerController: UIGestureRecognizerDelegate {
...
extension NativeImagePreviewViewerController: NativeImagePreviewViewerPageViewDelegate {
    func nativeImagePreviewViewerPageViewDidRequestDismiss(_ pageView: NativeImagePreviewViewerPageView) {
        guard pageView.index == currentIndex else { return }
        performDismissTransition()
    }

    func nativeImagePreviewViewerPageViewDidToggleVideoChrome(_ pageView: NativeImagePreviewViewerPageView) {
        guard pageView.index == currentIndex else { return }
        _ = pageView.toggleVideoControls()
        setNavigationBarHidden(shouldHideNavigationBarForCurrentPage, animated: true)
    }

    func nativeImagePreviewViewerPageView(_ pageView: NativeImagePreviewViewerPageView, setHorizontalPagingEnabled enabled: Bool) {
        guard pageView.index == currentIndex else { return }
        recalculateHorizontalPagingEnabled()
    }

    func nativeImagePreviewViewerPageViewDidBeginMultiTouch(_ pageView: NativeImagePreviewViewerPageView) {
        guard pageView.index == currentIndex else { return }
    ...
    func nativeImagePreviewViewerPageViewDidEndMultiTouch(_ pageView: NativeImagePreviewViewerPageView) {
        guard pageView.index == currentIndex else { return }
...
private final class NativeImagePreviewViewerPageCell: UICollectionViewCell {
    static let reuseIdentifier = "NativeImagePreviewViewerPageCell"

    let pageView = NativeImagePreviewViewerPageView()
