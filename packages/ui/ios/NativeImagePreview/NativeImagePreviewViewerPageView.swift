...
protocol NativeImagePreviewViewerPageViewDelegate: AnyObject {
...
final class NativeImagePreviewViewerPageView: UIView {
    weak var delegate: NativeImagePreviewViewerPageViewDelegate?

    private(set) var index: Int = 0
    private(set) var item: NativeImagePreviewMediaItem?
    ...
    private let videoPlayerView = NativeImagePreviewVideoPlayerView(frame: .zero)

    private var imageLoader: NativeImagePreviewImageLoading?
    ...
    func configure(
        index: Int,
        item: NativeImagePreviewMediaItem,
    ...
    func sharedElementState() -> NativeImagePreviewSharedElementState? {
        if isVideoPage {
            return posterImageView.nativeImagePreviewSharedElementState()
        }
        return imageView.nativeImagePreviewSharedElementState(clippingFrameInWindow: imageScrollView.frameInWindow())
    }

    func defaultTransitionGeometry(in windowBounds: CGRect) -> NativeImagePreviewSharedElementGeometry {
        ...
        let visibleFrame = NativeImagePreviewViewerTransitionCoordinator.aspectFitRect(for: imageSize, in: windowBounds)
    ...
    private func setupUI() {
        ...
        videoPlayerView.onControlsInteractStart = { [weak self] in
            ...
            self.delegate?.nativeImagePreviewViewerPageView(self, setHorizontalPagingEnabled: false)
        }
        videoPlayerView.onControlsInteractEnd = { [weak self] in
            guard let self = self else { return }
            self.delegate?.nativeImagePreviewViewerPageView(self, setHorizontalPagingEnabled: self.canPageHorizontally)
        }
    ...
    private func configureImagePage(image: UIImage?) {
        ...
        delegate?.nativeImagePreviewViewerPageView(self, setHorizontalPagingEnabled: canPageHorizontally)
    }
    ...
    private func configureVideoPage(url: URL, poster: URL?) {
        ...
        delegate?.nativeImagePreviewViewerPageView(self, setHorizontalPagingEnabled: true)
        updateVideoPlaybackIfNeeded()
    }
    ...
    private func handleImageSingleTap(_ recognizer: UITapGestureRecognizer) {
        delegate?.nativeImagePreviewViewerPageViewDidRequestDismiss(self)
    }
    ...
    private func handleVideoSingleTap(_ recognizer: UITapGestureRecognizer) {
        delegate?.nativeImagePreviewViewerPageViewDidToggleVideoChrome(self)
    }
    ...
    private func handlePinchState(_ recognizer: UIPinchGestureRecognizer) {
        ...
        case .began:
            ...
            delegate?.nativeImagePreviewViewerPageViewDidBeginMultiTouch(self)
        case .cancelled, .ended, .failed:
            if pinchInProgress {
                pinchInProgress = false
                delegate?.nativeImagePreviewViewerPageViewDidEndMultiTouch(self)
                delegate?.nativeImagePreviewViewerPageView(self, setHorizontalPagingEnabled: canPageHorizontally)
            }
        default:
            break
...
extension NativeImagePreviewViewerPageView: UIScrollViewDelegate {
...
extension NativeImagePreviewViewerPageView: UIGestureRecognizerDelegate {
